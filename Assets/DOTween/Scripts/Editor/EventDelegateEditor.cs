...
public static class EventDelegateEditor
{
	...
	{
		return Field(undoObject, del, true, Tools.minimalisticLook);
	}
	...
	static public bool Field (Object undoObject, EventDelegate del, bool removeButton, bool minimalistic)
	{
		...
		{
			if (!minimalistic) Tools.SetLabelWidth(82f);
		...
		if (remove)
		{
			Tools.RegisterUndo("Delegate Selection", undoObject);
			del.Clear();
			...
			if (ps != null)
			{
				for (int i = 0; i < ps.Length; ++i)
				{
					...
					if (selGO != null)
					{
						...
						int selection;
						string[] props = GetNames(ents, Tools.GetFuncName(param.obj, param.field), out selection);
	...
	static public string[] GetNames (List<Entry> list, string choice, out int index)
	{
		...
		{
			Entry ent = list[i];
			string del = Tools.GetFuncName(ent.target, ent.name);
			names[++i] = del;
	...
	{
		Field(undoObject, list, null, null, Tools.minimalisticLook);
	}
